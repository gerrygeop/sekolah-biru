		...
		<div class="container mx-auto px-4">
			<div class="flex items-center justify-between h-16">
				...
				<a href="{{ route('home') }}" class="flex items-center space-x-3">
					@if($globalSchoolProfile?->logo_path)
						<img src="{{ Storage::url($globalSchoolProfile->logo_path) }}"
							alt="{{ $globalSchoolProfile->school_name }}" class="w-10 h-10 rounded-lg object-contain">
					@else
						<div class="w-10 h-10 bg-primary-500 rounded-lg flex items-center justify-center">
							<span
								class="text-white font-bold text-xl">{{ substr($globalSchoolProfile?->school_name ?? 'S', 0, 1) }}</span>
						</div>
					@endif
					<div class="hidden sm:block">
						<div class="font-bold text-lg text-gray-900">
							{{ $globalSchoolProfile?->school_name ?? 'SMP Digital' }}</div>
				...
				<nav class="hidden lg:flex items-center space-x-1">
					...
					<div x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false" class="relative">
						...
						<div x-cloak x-show="open" x-transition:enter="transition ease-out duration-200"
							...
							class="absolute left-0 top-full w-56 bg-white rounded-lg shadow-lg border border-gray-200 py-2">
							...
							<a href="{{ route('profile.staff') }}"
								class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Tenaga
								Pendidik</a>
							...
							<a href="{{ route('profile.students') }}"
								class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Data
								Siswa</a>
						</div>
					</div>
					...
					<div x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false" class="relative">
						...
						<div x-cloak x-show="open" x-transition:enter="transition ease-out duration-200"
							x-transition:enter-start="opacity-0 translate-y-1"
							x-transition:enter-end="opacity-100 translate-y-0"
							class="absolute left-0 top-full w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-2">
	...
	<footer class="bg-gray-900 text-gray-300">
		<div class="container mx-auto px-4 py-12">
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
				...
				<div>
					<div class="flex items-center space-x-3 mb-4">
						...
						@endif
						<div>
							<div class="font-bold text-white">{{ $globalSchoolProfile?->school_name ?? 'SMP Digital' }}
							</div>
							<div class="text-xs text-gray-400">Unggul & Berkarakter</div>
						</div>
					</div>
					<p class="text-sm text-gray-400">
						{{ Str::limit($globalSchoolProfile?->vision ?? 'Menjadi sekolah unggul dalam prestasi dan berkarakter.', 120) }}
					</p>
				</div>
				...
				<div>
					...
					<ul class="space-y-2 text-sm">
						<li><a href="{{ route('profile.vision-mission') }}"
								class="hover:text-primary-400 transition">Visi & Misi</a>
						</li>
				...
				<div>
					...
					<ul class="space-y-2 text-sm">
						<li class="flex items-start">
							...
							<span>{{ $globalContactInfo?->address ?? $globalSchoolProfile?->address ?? 'Jl. Pendidikan No. 1, Jakarta' }}</span>
						</li>
						<li class="flex items-center">
							...
							<span>{{ $globalContactInfo?->email ?? $globalSchoolProfile?->email ?? 'info@sekolah.sch.id'
								}}</span>
						</li>
						<li class="flex items-center">
							<svg class="w-5 h-5 mr-2 text-primary-400" fill="none" stroke="currentColor"
							...
							<span>{{ $globalContactInfo?->phone ?? $globalSchoolProfile?->phone ?? '(021) 1234-5678' }}</span>
						</li>
					</ul>
				</div>
				...
				<div>
					...
					<div class="flex space-x-3">
						@if($globalContactInfo?->facebook_url)
							<a href="{{ $globalContactInfo->facebook_url }}" target="_blank" rel="noopener noreferrer"
						...
						@endif
						@if($globalContactInfo?->instagram_url)
							<a href="{{ $globalContactInfo->instagram_url }}" target="_blank" rel="noopener noreferrer"
								class="w-10 h-10 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-primary-500 transition">
								<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
									<path
						...
						@endif
						@if($globalContactInfo?->youtube_url)
							<a href="{{ $globalContactInfo->youtube_url }}" target="_blank" rel="noopener noreferrer"
								class="w-10 h-10 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-primary-500 transition">
								<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
									<path
						...
						@endif
						@if($globalContactInfo?->whatsapp)
							<a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $globalContactInfo->whatsapp) }}"
								target="_blank" rel="noopener noreferrer"
								class="w-10 h-10 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-green-500 transition">
								<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
									<path
			...
			<div class="border-t border-gray-800 mt-8 pt-8 text-center text-sm text-gray-400">
				<p>&copy; {{ date('Y') }} {{ $globalSchoolProfile?->school_name ?? 'SMP Digital' }}. All rights
					reserved.</p>
			</div>
		</div>
	</footer>
