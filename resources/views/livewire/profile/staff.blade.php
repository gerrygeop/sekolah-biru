	...
	<div class="container mx-auto px-4">
		...
		<div class="flex flex-wrap gap-2 justify-center mb-8">
			<button wire:click="setType('all')"
				class="px-6 py-2 rounded-lg font-medium transition {{ $selectedType === 'all' ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
				Semua
			</button>
			<button wire:click="setType('guru')"
				class="px-6 py-2 rounded-lg font-medium transition {{ $selectedType === 'guru' ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
				Guru
			</button>
			<button wire:click="setType('staff')"
				class="px-6 py-2 rounded-lg font-medium transition {{ $selectedType === 'staff' ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
				Staff
			</button>
			<button wire:click="setType('tenaga_kependidikan')"
				class="px-6 py-2 rounded-lg font-medium transition {{ $selectedType === 'tenaga_kependidikan' ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
				Tenaga Kependidikan
			</button>
		</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				@foreach ($employees as $employee)
					<div class="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 card-hover">
						...
						@else
							<div class="aspect-portrait bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center">
