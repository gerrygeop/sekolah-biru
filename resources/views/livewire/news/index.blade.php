			...
			<div class="flex flex-wrap gap-2 justify-center mb-8">
				...
				@foreach ($categories as $category)
					<button wire:click="setCategory({{ $category->id }})"
						class="px-6 py-2 rounded-lg font-medium transition {{ $selectedCategory === $category->id ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
						{{ $category->name }}
					</button>
				@endforeach
			</div>
					...
					<a href="{{ route('news.show', $item->slug) }}"
						...
						@else
							<div class="aspect-video bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center">
								<img src="{{ $item->featured_image_path }}" alt="{{ $item->title }}"
									class="w-full h-full object-cover group-hover:scale-105 transition duration-300">
							</div>
						@endif
						...
						<div class="p-5">
							<div class="flex items-center gap-2 mb-3">
								...
								<span class="text-xs text-gray-500">
									{{ $item->publish_date?->format('d M Y') ?? $item->created_at->format('d M Y') }}
								</span>
							</div>
