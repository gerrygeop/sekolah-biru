	...
	<div class="container mx-auto px-4">
			...
			<div class="flex flex-wrap gap-2 justify-center mb-8">
				@foreach ($academicYears as $year)
					<button wire:click="setYear({{ $year->id }})"
						class="px-6 py-2 rounded-lg font-medium transition {{ ($selectedYearId ?? $currentYear?->id) === $year->id ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
						{{ $year->year }}
						@if ($year->is_active)
		...
		@if ($distribution->isNotEmpty())
				...
				<div class="grid grid-cols-1 md:grid-cols-4 gap-6">
					<div class="text-center">
						<div class="text-3xl font-bold text-gray-900 mb-1">
							{{ number_format($distribution->sum('student_count')) }}</div>
						<div class="text-gray-600">Total Alumni</div>
					</div>
					<div class="text-center">
						<div class="text-3xl font-bold text-primary-600 mb-1">
							{{ number_format($groupedDistribution->get('SMA', collect())->sum('student_count')) }}
						</div>
						<div class="text-gray-600">SMA</div>
					</div>
					<div class="text-center">
						<div class="text-3xl font-bold text-secondary-600 mb-1">
							{{ number_format($groupedDistribution->get('SMK', collect())->sum('student_count')) }}
						</div>
						<div class="text-gray-600">SMK</div>
					</div>
					<div class="text-center">
						<div class="text-3xl font-bold text-green-600 mb-1">
							{{ number_format($groupedDistribution->get('MA', collect())->sum('student_count')) }}
						</div>
			...
			@foreach (['SMA', 'SMK', 'MA'] as $type)
				@if ($groupedDistribution->has($type))
						...
						<div class="overflow-x-auto">
							<table class="w-full">
								...
								<tbody class="divide-y divide-gray-200">
									@php
										$typeTotal = $groupedDistribution->get($type)->sum('student_count');
									@endphp
									@foreach ($groupedDistribution->get($type)->sortByDesc('student_count') as $school)
									...
									<tr class="bg-gray-50 font-semibold">
										...
										<td class="px-6 py-4 text-center">
											<span class="px-3 py-1 bg-primary-600 text-white rounded-full text-sm font-semibold">
												{{ number_format($typeTotal) }}
											</span>
										</td>
										<td class="px-6 py-4 text-center text-gray-900">
											{{ number_format(($typeTotal / $distribution->sum('student_count')) * 100, 1) }}%
										</td>
									</tr>
								</tbody>
							</table>
