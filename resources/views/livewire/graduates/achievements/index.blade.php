...
$achievements = computed(fn() => $studentService->getAllAchievements(12, $this->selectedCategory));
$categories = computed(fn() => \App\Models\Achievement::select('category')->distinct()->pluck('category'));
                ...
                <div class="flex flex-wrap gap-2 justify-center mb-8">
                    ...
                    @foreach($this->categories as $category)
                        <button wire:click="setCategory('{{ $category }}')"
                            class="px-6 py-2 rounded-lg font-medium transition {{ $selectedCategory === $category ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50' }}">
                            {{ $category }}
                        </button>
                    @endforeach
                </div>
