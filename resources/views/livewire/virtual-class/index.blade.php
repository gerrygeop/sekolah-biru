	...
	<div class="container mx-auto px-4">
		...
		<div class="flex flex-wrap gap-2 justify-center mb-8">
			<button wire:click="setGrade(null)"
				class="px-6 py-2 rounded-lg font-medium transition {{ $selectedGrade === null ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
				Semua
			</button>
			<button wire:click="setGrade('7')"
				class="px-6 py-2 rounded-lg font-medium transition {{ $selectedGrade === 7 ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
				Kelas 7
			</button>
			<button wire:click="setGrade('8')"
				class="px-6 py-2 rounded-lg font-medium transition {{ $selectedGrade === 8 ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
				Kelas 8
			</button>
			<button wire:click="setGrade('9')"
				class="px-6 py-2 rounded-lg font-medium transition {{ $selectedGrade === 9 ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-primary-200' }} cursor-pointer">
				Kelas 9
			</button>
		</div>

		@if ($virtualClasses->isNotEmpty())
